...
import { getFlightBySlug } from '../../../Utils/Requests/Flight';
import Loading from '../../core/Loading';

function FlightCaller(props) {
	const [sold, setSold] = useState([]);
	const [booked, setBooked] = useState([]);
	const [numberOfSeats, setNumberOfSeats] = useState(0);
	const [error, setError] = useState('');
	const [details, setDetails] = useState({ name: 'Flight' });
	...
	const fetchAllBookingDetails = async () => {
		const resp = await getFlightBySlug(props.match.params.slug).catch(err => {
...
export default withRouter(FlightCaller);
