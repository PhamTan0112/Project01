...
import { addNewFlight, updateFlight } from "../../../Utils/Requests/Flight";
...
class Success extends Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.isUpdate) {
    	const resp = await updateFlight(this.props.match.params.slug, formData).catch(err => {
    	...
    	// Add the flight
    	const resp = await addNewFlight(formData).catch(err => {
    		this.setState({ loading: false, error: err.response.data.error });
    	});
    	if (resp && resp.status === 200) {
    		this.setState({ loading: false });
  ...
  renderMessage = () => {
    ...
    if (error) {
      ...
      Swal.fire({
        type: "success",
        title: `Successfully ${message} the flight!`,
      });
      return <Redirect to="/" />;
    }
  };
