...
class Home extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({ user });
    this.fetchAllFlightData();
    this.fetchAllPeopleData();
    this.fetchMyFlightData();
  ...
  fetchAllFlightData = async () => {
    let availablecount = 0;
    let unavailablecount = 0;
    const avialable = await getAllAvailableFlightes();
    if (avialable && avialable.status === 200) {
      availablecount = avialable.data.length;
    }
    const unavailable = await getAllUnavailableFlightes();
  ...
  fetchMyFlightData = async () => {
    let availablecount = 0;
    let unavailablecount = 0;
    const avialable = await getAvailableFlightesOfOwner();
    if (avialable && avialable.status === 200) {
      availablecount = avialable.data.length;
    }
    const unavailable = await getUnavailableFlightesOfOwner();
    if (unavailable && unavailable.status === 200) {
      unavailablecount = unavailable.data.length;
    }
    this.setState({
          ...
          {role === "superadmin" && (
            <div className="row">
              <div className="col-md-6">
                <h3>Total Flightes</h3>
                <Doughnut
                  data={totalFlight}
                  height={20}
                  width={50}
                  onElementsClick={e => {
                    e[0]._url = ["all-flight-available", "all-flight-unavailable"];
          ...
          <div className="row">
            <div className="col-md-6">
              ...
              <Doughnut
                ...
                width={50}
                onElementsClick={e => {
                  e[0]._url = ["flight-available", "flight-unavailable"];
                  this.handleRedirect(e[0]);
                }}
              />
            </div>
