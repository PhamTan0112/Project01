...
import { searchFlightByFilter } from "../../actions/location";

const Filters = ({ info, setFlightes, setLoading }) => {
  ...
  const onChangeCheckbox = e => {
    ...
    // Logic for getting all flight if all checkbox are unchecked
    const newArr =[];
    if (list.length <= 0) {
      travels.map((tr, i) => {
        newArr[i] = tr._id;
  ...
  const fetchCheckedTravels = async (travel, type) => {
    const resp = await searchFlightByFilter({ travel, ...info, type });
    setFlightes(resp);
    setLoading(false);
  };
